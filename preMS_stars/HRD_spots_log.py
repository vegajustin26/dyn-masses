...
Llims = [-1.8, 1.1]     # log Lstar / Lsun
Tlims = [3.66, 3.50]      # log Teff / K
...
for i in range(len(age)):
    ax.plot(iso_T[iso_AGE == age[i]], iso_L[iso_AGE == age[i]], 'b')
...
for i in range(len(ms)):
    ...
    ax.plot(mt_T[mt_AGE <= 0.05], mt_L[mt_AGE <= 0.05], 'r')
...
ax.plot(db['logTeff'][base], db['logLs'][base], '.k', zorder=0)
...
for i in range(len(db['NAME'][M3])): print(db['NAME'][M3][i])
...
for il in range(len(xspt)):
    ax.annotate(lspt[il], xy=(xspt[il], 0.95), ha='center', size=10, 
                color='gray')
