...
for ext in ['.image', '.mask', '.model', '.pb', '.psf', '.residual', '.sumwt']:
    os.system('rm -rf im_'+fname+'_nf'+ext)
tclean(vis=fname+'.sim/'+fname+'.sim.'+cfg+'.ms', imagename='im_'+fname+'_nf', 
       specmode='cube', start='-6.0km/s', width='0.1km/s', nchan=121,
       outframe='LSRK', restfreq='230.538GHz', imsize=512, cell='0.02arcsec', 
       deconvolver='multiscale', scales=cleanscales,
       niter=5000000, threshold='7mJy', weighting='briggs', robust=0.5, 
